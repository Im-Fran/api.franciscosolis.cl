 ...
 */
/**
 * Verifies an access token against the auth Worker's published keys.
 *
 * `allowedAudiences` is a parameter rather than a fixed env var because this Worker accepts two
 * different populations — the support team through `SUPPORT_ALLOWED_AUDIENCES`, and a person reading
 * their own ticket through the wider `SUPPORT_REQUESTER_AUDIENCES`. Passing the list in at the call
 * site is what keeps those two gates from being accidentally merged into one.
 */
