 ...
 * binding by assignment is the same move `captureEmail` makes with `EMAIL` and `stubAi` with `AI`,
 * and it reaches the Worker behind `SELF` for the same reason: both read the same `env`.
 *
