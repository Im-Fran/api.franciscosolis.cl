...
/**
 * Mints the access tokens this Worker verifies offline.
 *
...
const DEFAULT_KID = 'support-test-key'
...
/** Claims the auth Worker puts on an access token, with everything the agent gate wants. */
const agentClaims = (overrides: Partial<AccessTokenClaims> = {}): AccessTokenClaims => {
  const now = Math.floor(Date.now() / 1000)
  return {
    iss: 'https://auth.test',
    sub: 'agent-1',
    aud: 'franciscosolis-support',
    exp: now + 900,
    iat: now - 5,
    jti: 'jti-1',
    sid: 'session-1',
    ...
    picture: null,
    roles: ['agent'],
    permissions: ['support:agent', 'support:admin'],
    ...overrides,
  }
}

/**
 * Claims for somebody who merely has an account on the website — the other population this Worker
 * accepts. Deliberately minted for a different `aud` and carrying no support permission, because
 * the whole point of the two audience lists is that this token must never open `/admin`.
 */
const requesterClaims = (overrides: Partial<AccessTokenClaims> = {}): AccessTokenClaims =>
  agentClaims({
    sub: 'visitor-1',
...
const mintToken = async (overrides: Partial<AccessTokenClaims> = {}, signWith?: Jwk): Promise<string> => {
  ...
  return sign(agentClaims(overrides) as unknown as Record<string, unknown>, key as never, 'EdDSA')
}
...
/** Publishes the file's public key and returns headers carrying a token an agent would send. */
const asAgent = async (overrides: Partial<AccessTokenClaims> = {}): Promise<Record<string, string>> => {
...
/** The same, for a signed-in member of the public. */
const asRequester = async (overrides: Partial<AccessTokenClaims> = {}): Promise<Record<string, string>> => {
  const { publicJwk } = await testKeyPair()
  stubJwks([publicJwk])
  return {
    Authorization: `Bearer ${await mintToken(requesterClaims(overrides))}`,
    'Content-Type': 'application/json',
  }
}

/** Headers carrying the per-ticket link secret instead of an account. */
const asLinkHolder = (secret: string): Record<string, string> => ({
  Authorization: `Ticket ${secret}`,
  'Content-Type': 'application/json',
})
