...
const DEFAULT_KID = 'marketplace-test-key'
...
/** Claims the auth Worker puts on an access token, with everything the editor gate wants. */
const editorClaims = (overrides: Partial<AccessTokenClaims> = {}): AccessTokenClaims => {
  const now = Math.floor(Date.now() / 1000)
  return {
    iss: 'https://auth.test',
    sub: 'editor-1',
    aud: 'franciscosolis-marketplace',
    exp: now + 900,
    iat: now - 5,
    jti: 'jti-1',
    sid: 'session-1',
