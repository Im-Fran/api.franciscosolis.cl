...
describe('the tab registry', () => {
  it('recognises exactly the five tabs a page is built from', () => {
    expect([...TAB_KEYS]).toEqual(['overview', 'releases', 'wiki', 'reviews', 'contact'])
...
describe('normalizeTabs', () => {
  it('keeps the order an editor chose', () => {
    expect(normalizeTabs(['overview', 'wiki', 'releases'])).toEqual(['overview', 'wiki', 'releases'])
  })

  it('forces the overview tab in front, whatever was sent', () => {
    expect(normalizeTabs(['contact', 'overview'])).toEqual(['overview', 'contact'])
    expect(normalizeTabs(['releases'])).toEqual(['overview', 'releases'])
  ...
  it('drops an unknown key rather than failing the whole list', () => {
    expect(normalizeTabs(['releases', 'forum', 'wiki'])).toEqual(['overview', 'releases', 'wiki'])
  })
})
...
describe('parseTabs', () => {
   ...
   * page a tab rather than 500-ing the product it was left on.
   */
  it.each([
    ['null', null],
    ['an empty string', ''],
