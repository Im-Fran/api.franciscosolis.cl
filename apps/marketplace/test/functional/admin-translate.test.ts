 ...
 *
 * One route serves all three kinds of row here — a product page, a release note, a wiki page —
...
describe('POST /admin/translate', () => {
  ...
  it('writes nothing to the record it translated for', async () => {
    ...
    const rows = await env.DB.prepare('SELECT COUNT(*) AS total FROM products').first<{ total: number }>()
  ...
  it('refuses a draft the API would reject on save', async () => {
    // Over the 120-character cap on a product's name. Offering it would hand the editor a
