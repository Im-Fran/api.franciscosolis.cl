...
import { clearDatabase, countRows, seedProduct, seedWikiPage } from '../helpers/db'
...
describe('POST /admin/products/:productId/wiki', () => {
  beforeEach(clearDatabase)

  const post = (productId: string, payload: Record<string, unknown>) =>
    admin(`/products/${productId}/wiki`, { method: 'POST', body: JSON.stringify(payload) })
  ...
  it('nests a page under a top-level section', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const section = await seedWikiPage({ productId: app.id, slug: 'overview' })
  ...
  it('refuses a duplicate slug inside the product, with a 409', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    await seedWikiPage({ productId: app.id, slug: 'install' })

    expect((await post(app.id, { title: 'Install', slug: 'install' })).status).toBe(409)
  })

  it('allows the same slug in a different product', async () => {
    const one = await seedProduct({ slug: 'one' })
    const two = await seedProduct({ slug: 'two' })
    await seedWikiPage({ productId: one.id, slug: 'install' })
...
describe('the wiki hierarchy rules', () => {
  ...
  const patch = (productId: string, id: string, payload: Record<string, unknown>) =>
    admin(`/products/${productId}/wiki/${id}`, { method: 'PATCH', body: JSON.stringify(payload) })

  it('refuses a parent belonging to another product', async () => {
    const mine = await seedProduct({ slug: 'mine' })
    const theirs = await seedProduct({ slug: 'theirs' })
    const foreign = await seedWikiPage({ productId: theirs.id, slug: 'overview' })

    const response = await post(mine.id, { title: 'Commands', parent_id: foreign.id })

    expect(response.status).toBe(422)
    expect(await errorOf(response)).toContain('does not belong to this product')
  })
  ...
  it('refuses nesting under a page that is itself nested', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const section = await seedWikiPage({ productId: app.id, slug: 'overview' })
    const child = await seedWikiPage({ productId: app.id, slug: 'commands', parentId: section.id })
  ...
  it('refuses making a page its own parent', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const page = await seedWikiPage({ productId: app.id, slug: 'overview' })
  ...
  it('refuses nesting a page that already has pages under it', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const section = await seedWikiPage({ productId: app.id, slug: 'overview' })
    await seedWikiPage({ productId: app.id, slug: 'commands', parentId: section.id })
    const other = await seedWikiPage({ productId: app.id, slug: 'install' })
...
describe('GET /admin/products/:productId/wiki', () => {
  beforeEach(clearDatabase)

  it('lists every page flat, in sidebar order, drafts included', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    await seedWikiPage({ productId: app.id, slug: 'second', position: 1 })
    await seedWikiPage({ productId: app.id, slug: 'first', position: 0, status: 'draft' })

    const data = await json<{ slug: string }[]>(await admin(`/products/${app.id}/wiki`))
  ...
  it('nests the same pages when asked for the tree, and drops the bodies', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const section = await seedWikiPage({ productId: app.id, slug: 'overview', position: 0 })
    await seedWikiPage({ productId: app.id, slug: 'commands', parentId: section.id, position: 1 })

    const tree = await json<{ slug: string; children: { slug: string }[] }[]>(
      await admin(`/products/${app.id}/wiki?tree=true`),
    )
...
describe('POST /admin/products/:productId/wiki/reorder', () => {
  beforeEach(clearDatabase)

  it('applies the new order', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const first = await seedWikiPage({ productId: app.id, slug: 'first', position: 0 })
    const second = await seedWikiPage({ productId: app.id, slug: 'second', position: 1 })

    const data = await json<{ slug: string }[]>(
      await admin(`/products/${app.id}/wiki/reorder`, {
        method: 'POST',
  ...
  /** Scoped by product: a foreign id is ignored rather than reordering someone else's sidebar. */
  it('ignores an id belonging to another product', async () => {
    const mine = await seedProduct({ slug: 'mine' })
    const theirs = await seedProduct({ slug: 'theirs' })
    const foreign = await seedWikiPage({ productId: theirs.id, slug: 'overview', position: 0 })

    await admin(`/products/${mine.id}/wiki/reorder`, {
      method: 'POST',
      body: JSON.stringify({ items: [{ id: foreign.id, position: 99 }] }),
    })

    const [row] = await json<{ position: number }[]>(await admin(`/products/${theirs.id}/wiki`))
...
describe('DELETE on a wiki page', () => {
  ...
  it('promotes the pages nested under it instead of deleting them', async () => {
    ...
    const response = await admin(`/products/${app.id}/wiki/${section.id}`, { method: 'DELETE' })

    expect(response.status).toBe(204)
    expect(await countRows('product_wiki_pages')).toBe(1)

    const data = await json<{ id: string; parent_id: string | null }>(
      await admin(`/products/${app.id}/wiki/${child.id}`),
    )
    expect(data.parent_id).toBeNull()
  })

  it('will not delete a page through the wrong product', async () => {
    const mine = await seedProduct({ slug: 'mine' })
    const theirs = await seedProduct({ slug: 'theirs' })
    const page = await seedWikiPage({ productId: theirs.id, slug: 'secrets' })

    expect((await admin(`/products/${mine.id}/wiki/${page.id}`, { method: 'DELETE' })).status).toBe(404)
  })
})
