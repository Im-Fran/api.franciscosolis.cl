...
import { clearDatabase, readAuditLog, seedProduct, seedPurchase, seedVoucher } from '../helpers/db'
...
const seedSale = async () => {
  const app = await seedProduct({ slug: 'openbattery', name: 'OpenBattery' })
  const sale = await seedPurchase({ productId: app.id, productSlug: app.slug, environment: 'sandbox' })
  return { app, sale }
}

describe('POST /admin/products/:productId/sales/:saleId/vouchers', () => {
  const issue = (productId: string, saleId: string, payload: Record<string, unknown> = {}) =>
    admin(`/products/${productId}/sales/${saleId}/vouchers`, {
      method: 'POST',
  ...
  it('voids the live voucher it replaces, keeping it on the record', async () => {
    ...
    const vouchers = await data<{ id: string; status: string; void_reason: string | null }[]>(
      await admin(`/products/${app.id}/vouchers`),
    )
  ...
  it('issues to another address without touching the sale', async () => {
    ...
    const rows = await data<{ email: string }[]>(await admin(`/products/${app.id}/sales`))
  ...
  it('404s for a sale of another product', async () => {
...
describe('POST /admin/products/:productId/vouchers/:voucherId/send', () => {
  const send = (productId: string, voucherId: string, payload: Record<string, unknown> = {}) =>
    admin(`/products/${productId}/vouchers/${voucherId}/send`, {
      method: 'POST',
      body: JSON.stringify(payload),
    })
  ...
  it('re-sends to the address it was issued to and counts the send', async () => {
    const { app, sale } = await seedSale()
    const voucher = await seedVoucher({ purchaseId: sale.id, productId: app.id, productSlug: app.slug })
  ...
  it('does not count a send that failed', async () => {
    ...
    const row = await data<{ sent_count: number }>(await admin(`/products/${app.id}/vouchers/${voucher.id}`))
...
describe('POST /admin/products/:productId/vouchers/:voucherId/void', () => {
  it('voids with a reason and keeps the row', async () => {
    ...
    const voided = await data<{ status: string; voided_by: string; void_reason: string; voided_at: string }>(
      await admin(`/products/${app.id}/vouchers/${voucher.id}/void`, {
        method: 'POST',
  ...
  it('keeps the first reason and the first date when voided twice', async () => {
    const { app, sale } = await seedSale()
    const voucher = await seedVoucher({ purchaseId: sale.id, productId: app.id, productSlug: app.slug })
    const url = `/products/${app.id}/vouchers/${voucher.id}/void`
...
describe('GET /me/vouchers', () => {
  it('gives a buyer their own receipts, void ones included, and nobody else\'s', async () => {
    const { app, sale } = await seedSale()
    await seedVoucher({ purchaseId: sale.id, productId: app.id, productSlug: app.slug, number: 'FS-2026-000001' })
    await seedVoucher({
      purchaseId: sale.id,
      productId: app.id,
      productSlug: app.slug,
    ...
    const rows = await data<{ number: string; status: string }[]>(
      await SELF.fetch('https://marketplace.test/me/vouchers', { headers: await asBuyer() }),
    )
  ...
  it('requires a token', async () => {
    expect((await SELF.fetch('https://marketplace.test/me/vouchers')).status).toBe(401)
  })
})
