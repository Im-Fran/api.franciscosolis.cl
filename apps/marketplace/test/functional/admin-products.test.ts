...
import { clearDatabase, countRows, readAuditLog, seedProduct } from '../helpers/db'
...
const create = (payload: Record<string, unknown>) =>
  admin('/products', { method: 'POST', body: JSON.stringify(payload) })
...
describe('POST /admin/products', () => {
  ...
  it('writes an audit row naming the editor', async () => {
    ...
    expect(entry?.event).toBe('product.created')
    expect(entry?.actor_email).toBe('fran@franciscosolis.cl')
    expect(entry?.resource_type).toBe('products')
  })
})
...
describe('GET /admin/products', () => {
  ...
  it('shows drafts, which the public listing never does', async () => {
    await seedProduct({ slug: 'draft-one', status: 'draft' })
    await seedProduct({ slug: 'live-one', status: 'published' })

    const data = await json<{ slug: string }[]>(await admin('/products'))
  ...
  it('narrows on status', async () => {
    ...
    const data = await json<{ slug: string }[]>(await admin('/products?status=draft'))
  ...
  it('answers in the default locale and hands back the translation map unresolved', async () => {
    await seedProduct({ slug: 'openbattery', name: 'OpenBattery', translations: '{"es":{"name":"Batería"}}' })

    const [row] = await json<{ name: string; translations: Record<string, unknown> }[]>(await admin('/products'))
...
describe('PATCH /admin/products/:id', () => {
  ...
  const patch = (id: string, payload: Record<string, unknown>) =>
    admin(`/products/${id}`, { method: 'PATCH', body: JSON.stringify(payload) })

  it('leaves an omitted field alone and clears one sent as null', async () => {
    const app = await seedProduct({ slug: 'openbattery', tagline: 'Telemetry', summary: 'A summary' })
  ...
  it('replaces the tabs wholesale rather than merging them', async () => {
    const app = await seedProduct({ slug: 'openbattery', tabs: '["overview","releases","wiki"]' })
  ...
  /**
   * `published_at` records when the product was announced, not when its status last moved, so
  ...
  it('keeps the original published_at across an unpublish and a republish', async () => {
    ...
    const app = await seedProduct({ slug: 'openbattery', status: 'published', publishedAt: announced })
  ...
  it('refuses a slug another product already holds', async () => {
    await seedProduct({ slug: 'taken' })
    const app = await seedProduct({ slug: 'mine' })
...
describe('POST /admin/products/reorder', () => {
  beforeEach(clearDatabase)

  it('applies the new order and answers with the list', async () => {
    const first = await seedProduct({ slug: 'first', position: 0 })
    const second = await seedProduct({ slug: 'second', position: 1 })

    const data = await json<{ slug: string }[]>(
      await admin('/products/reorder', {
        method: 'POST',
  ...
  it('is not swallowed by the :id route', async () => {
    const response = await admin('/products/reorder', {
      method: 'POST',
...
describe('DELETE /admin/products/:id', () => {
  ...
  it('takes the release notes and the wiki pages with it', async () => {
    ...
    await SELF.fetch(`https://marketplace.test/admin/products/${app.id}/releases`, {
      method: 'POST',
      headers: await asEditor(),
      body: JSON.stringify({ version: '1.0.0', title: 'First' }),
    })
    await SELF.fetch(`https://marketplace.test/admin/products/${app.id}/wiki`, {
      method: 'POST',
      headers: await asEditor(),
      body: JSON.stringify({ title: 'Installation' }),
    })

    expect(await countRows('product_releases')).toBe(1)
    expect(await countRows('product_wiki_pages')).toBe(1)

    const response = await admin(`/products/${app.id}`, { method: 'DELETE' })

    expect(response.status).toBe(204)
    expect(await countRows('products')).toBe(0)
    expect(await countRows('product_releases')).toBe(0)
    expect(await countRows('product_wiki_pages')).toBe(0)
  })

  it('answers 404 for a product that does not exist', async () => {
    expect((await admin(`/products/${crypto.randomUUID()}`, { method: 'DELETE' })).status).toBe(404)
  })
})
