...
import { clearDatabase, countRows, readAuditLog, seedProduct, seedPurchase } from '../helpers/db'
import { asBuyer, asEditor } from '../helpers/tokens'

/**
 * The sales console of one product: recording a sale taken outside MercadoPago, adding it up,
 * and giving it back.
 *
...
describe('POST /admin/products/:productId/sales', () => {
  ...
  it('records a cash sale as an approved payment, attributed to the editor', async () => {
    const app = await seedProduct({ slug: 'openbattery', pricingMode: 'paid', priceAmount: 4990 })
  ...
  it('entitles the recipient even though the sale carries no account id', async () => {
    ...
    const access = await data<{ has_paid: boolean; can_download: boolean }>(
      await SELF.fetch('https://marketplace.test/products/openbattery/access', {
...
describe('GET /admin/products/:productId/sales', () => {
  it('lists only this product\'s sales, with its withdrawal window on each', async () => {
    const mine = await seedProduct({ slug: 'openbattery' })
    const other = await seedProduct({ slug: 'other' })
    await seedPurchase({ productId: mine.id, productSlug: mine.slug })
    await seedPurchase({ productId: other.id, productSlug: other.slug })

    const rows = await data<{ withdrawal: { deadline: string | null } }[]>(
      await admin(`/products/${mine.id}/sales`),
    )
  ...
  it('narrows by source, which is what separates cash from card', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    await seedPurchase({ productId: app.id, productSlug: app.slug })
    await seedPurchase({ productId: app.id, productSlug: app.slug, source: 'cash', provider: 'manual' })

    const rows = await data<{ source: string }[]>(await admin(`/products/${app.id}/sales?source=cash`))
  ...
  it('404s for a product that does not exist', async () => {
    expect((await admin('/products/nope/sales')).status).toBe(404)
  })
})

describe('GET /admin/products/:productId/sales/summary', () => {
  it('separates what arrived from what went back out', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    await seedPurchase({ productId: app.id, productSlug: app.slug, amount: 4990 })
    await seedPurchase({ productId: app.id, productSlug: app.slug, amount: 4990, email: 'two@example.com' })
    await seedPurchase({
      productId: app.id,
    ...
    const summary = await data<{
      ...
      environment: string
    }>(await admin(`/products/${app.id}/sales/summary`))
  ...
  it('totals only the rows its filters select', async () => {
    ...
    const summary = await data<{ gross: number }>(
      await admin(`/products/${app.id}/sales/summary?source=gift`),
    )
...
describe('POST /admin/products/:productId/sales/:saleId/refund', () => {
  const refund = (productId: string, saleId: string, payload: Record<string, unknown>) =>
    admin(`/products/${productId}/sales/${saleId}/refund`, {
      method: 'POST',
  ...
  it('leaves the sale approved when the provider refuses', async () => {
    ...
    const rows = await data<{ status: string }[]>(await admin(`/products/${app.id}/sales`))
  ...
  it('refuses a sale taken in the other environment', async () => {
    ...
    const sale = await seedPurchase({ productId: app.id, productSlug: app.slug, environment: 'live' })
  ...
  it('ends the entitlement it was granting', async () => {
    ...
    const access = await data<{ can_download: boolean }>(
      await SELF.fetch('https://marketplace.test/products/openbattery/access', { headers: await asBuyer() }),
    )

    expect(access.can_download).toBe(false)
  })

  it('404s for a sale belonging to another product', async () => {
    const mine = await seedProduct({ slug: 'openbattery' })
    const other = await seedProduct({ slug: 'other' })
    const sale = await seedPurchase({ productId: other.id, productSlug: other.slug })

    expect((await refund(mine.id, sale.id, { reason: 'other' })).status).toBe(404)
  })
})

describe('PATCH /admin/products/:productId/sales/:saleId', () => {
  it('corrects the address and attaches the account, and nothing else', async () => {
    ...
    const updated = await data<{ email: string; user_id: string; linked_to_account: boolean; amount: number }>(
      await admin(`/products/${app.id}/sales/${sale.id}`, {
        method: 'PATCH',
...
describe('GET /admin/products/:productId/sales/:saleId', () => {
  it('says whether the sale can be refunded, and which rule refused it', async () => {
    const app = await seedProduct({ slug: 'openbattery' })
    const live = await seedPurchase({ productId: app.id, productSlug: app.slug, environment: 'live' })

    const detail = await data<{ refund: { refundable: boolean; reason: string | null; withdrawal_days: number } }>(
      await admin(`/products/${app.id}/sales/${live.id}`),
    )
