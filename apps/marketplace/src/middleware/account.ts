 ...
 * account's own downloads. The audience list is `MARKETPLACE_ACCOUNT_AUDIENCES` — a separate variable from
...
const readAccount = async (c: Context<AppEnv>): Promise<Account> => {
  ...
  try {
    claims = await verifyAccessToken(c.env, token, c.env.MARKETPLACE_ACCOUNT_AUDIENCES)
  } catch (error) {
 ...
 * needs the identity (a paid product, a purchase history) reaches for `requireAccount` instead.
 */
...
const tryAccount = async (c: Context<AppEnv>): Promise<Account | null> => {
  ...
  try {
    const claims = await verifyAccessToken(c.env, token, (c.env as Env).MARKETPLACE_ACCOUNT_AUDIENCES)
