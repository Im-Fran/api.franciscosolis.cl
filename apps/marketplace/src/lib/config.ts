...
/** Publication states shared by products, releases and wiki pages. */
 ...
 * CPU: it is the cheap ceiling on what a loop in the CMS front-end — which is where this content is
 * edited — can cost. It is set well above what translating a whole product page by hand takes,
 ...
 * and that is the number that decides whether a product should be sold at all.
 */
 ...
 * minted, so revoking an editor takes effect within one access-token lifetime rather than instantly.
 * The domain check runs on every request and is the harder boundary.
 */
const EDITOR_PERMISSION = 'marketplace:editor'
