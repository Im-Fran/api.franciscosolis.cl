 ...
 * product credential is spelled exactly like a production one, so "does the token start with
...
const createPreference = async (env: Env, input: CreatePreferenceInput): Promise<Preference> =>
  request<Preference>(env, '/checkout/preferences', {
    ...
    body: JSON.stringify({
      ...
      // product page rather than on a MercadoPago receipt they have to close.
