/**
 * The languages a product page is published in, and the rules for serving them.
 *
 ...
 * product (its slug, its ordering, its links, its banner, the version number of a release) is
 ...
 * Three different kinds of row are translated here — a product, a release note, a wiki page —
...
/**
 * Prose fields of a product a translation may override.
 *
 * The two tab bodies are in here and the tab *keys* are not: which tabs a product has is
 * structure, and a page that shows a Wiki tab in English and not in Spanish is a bug rather than a
 * translation.
 */
const PRODUCT_TRANSLATABLE_FIELDS = ['name', 'tagline', 'summary', 'overview_body', 'contact_body'] as const

/** Same, for a release note. The version label is not prose — `2.6.4` is `2.6.4` in every language. */
const RELEASE_TRANSLATABLE_FIELDS = ['title', 'body'] as const
