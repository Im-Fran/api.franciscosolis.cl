 ...
 *
 * Read-only, and cross-product: it is the "everything that ever came in" screen, whatever product
 * it came in for. Administering the sales *of one product* — recording one taken in cash, issuing
 * and re-sending its voucher, refunding it — is `routes/admin/sales.ts`, which is nested under the
 * product for the same reason the releases and the wiki are.
 *
 * Nothing here writes, and that is not because writing a payment is forbidden outright: it is because
 * this listing spans products and a total over it is the only thing it is for. The narrow set of
...
app.get(
  ...
  describeRoute({
    description:
      'Every payment taken through this service, newest first, narrowed by product, status or buyer address. Read-only: a payment\'s status is the provider\'s to change, and it arrives over the webhook.',
    tags: ['Admin · Store'],
    security: [{ bearerAuth: [] }],
    responses: {
  ...
  async (c) => {
    const { product_id: productId, status, email, limit = PAGINATION.defaultLimit, offset = 0 } = c.req.valid('query')

    const rows = await listPurchases(getDb(c.env), { productId, status, email, limit, offset })
