...
/**
 * The money half of a standalone product page: what it costs, whether this person has paid, how
 ...
 * - `POST …/checkout` and `/me/*` **require** an account, from `MARKETPLACE_ACCOUNT_AUDIENCES`.
 *
...
/** Same rule as every other public read here: a draft product does not exist. */
...
app.get(
  ...
  describeRoute({
    description:
      'What a product costs: its pricing mode, the price of a paid one, the amount a donation prefills and the smallest amount checkout accepts. Public and cacheable — it is the same answer for every visitor. Whether a *particular* person has paid is `GET /products/:slug/access`.',
    tags: ['Store'],
    responses: {
      200: {
  ...
  async (c) => {
    ...
    const { pricing } = await resolveAccess(getDb(c.env), product, undefined)
...
const checkoutSchema = v.object({
  /**
   * What to charge, in whole pesos. Only meaningful for a donation: a paid product is charged
...
app.post(
  ...
  describeRoute({
    ...
    responses: {
      ...
      404: { description: 'No published product with that slug' },
      409: { description: 'This account has already paid for the product' },
      422: { description: 'The product takes no payments, or the amount is out of bounds' },
  ...
  async (c) => {
    ...
    const product = await requirePublishedProduct(db, c.req.param('slug'))
    const access = await resolveAccess(db, product, account)

    if (access.has_paid) {
      // A donation is a repeatable act and a purchase is not, so only the second is refused: paying
      // twice for the same product is a mistake somebody wants their money back for.
      if (access.pricing.mode === 'paid') {
        throw new HTTPException(409, { message: 'This account has already bought this product' })
      }
    }
    ...
    const returnUrl = `${site}${body.return_path ?? `/product/${product.slug}`}`

    let preference
    try {
      preference = await createPreference(c.env, {
        externalReference: purchase.externalReference,
        title: kind === 'donation' ? `Support ${product.name}` : product.name,
        description:
          kind === 'donation'
            ? `Voluntary payment for ${product.name}`
            : `Licence for ${product.name}`,
        amount: charge.amount,
        ...
        metadata: { purchase_id: purchase.id, product_id: product.id, kind },
      })
    } catch (error) {
