...
type ReleaseFile = typeof productReleaseFiles.$inferSelect

/**
 * One downloadable build as the Releases tab renders it.
 *
...
const listOrder = [asc(productReleaseFiles.position), asc(productReleaseFiles.filename)]

const listReleaseFiles = async (db: Database, filters: ReleaseFileFilters): Promise<ReleaseFile[]> => {
  const clauses = [eq(productReleaseFiles.releaseId, filters.releaseId)]
  if (filters.status) {
    clauses.push(eq(productReleaseFiles.status, filters.status))
  }

  return db
    .select()
    .from(productReleaseFiles)
    .where(and(...clauses))
    .orderBy(...listOrder)
}

/** Every published file of a product, whatever release it hangs off. What a download path needs. */
const findReleaseFileById = async (db: Database, id: string): Promise<ReleaseFile | null> => {
  const [file] = await db.select().from(productReleaseFiles).where(eq(productReleaseFiles.id, id)).limit(1)
  return file ?? null
}
...
const countDownload = async (db: Database, id: string) => {
  await db
    ...
    .set({ downloadCount: sql`${productReleaseFiles.downloadCount} + 1` })
    .where(eq(productReleaseFiles.id, id))
}
