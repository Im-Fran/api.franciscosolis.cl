...
- **Cloudflare Queues**, producer only: `NOTIFICATIONS_QUEUE` → `franciscosolis-notifications`
  (`-dev` under `env.dev`), drained by `apps/notifications`. It feeds the bell on the website and is
...
- `src/services/` — `products.ts`, `releases.ts`, `wiki.ts`, `compatibility.ts`, `reviews.ts`,
  ...
  `sales.ts`, `vouchers.ts`, `mail.ts`, `audit.ts`, `ai.ts`, `notify.ts` (the notifications-queue
  producer), and `access.ts` — the one place that decides whether somebody may download.
...
- **Four events, published beside the mail and never instead of it** (`services/notify.ts`):
  `purchase_completed` (webhook approval, or a sale recorded by hand), `purchase_refunded` (the
  editorial refund, or one made in the provider's console that only the webhook sees),
  `release_published` and `review_reply`. The voucher and the refund notice are sent exactly as
  before; the notifications Worker knows the first two types as not emailable.
- **An event needs an account.** `isLinkedToAccount` is the gate, so a hand-recorded sale carrying
  `UNLINKED_USER_ID` publishes nothing, and the release fan-out excludes the sentinel in SQL.
- **Transitions, not deliveries.** The webhook publishes on the move *into* `approved` or `refunded`,
  compared with the row as it was before the notification, because a fresh event key is not a fresh
  status. A release announces itself the first time it is published — `published_at` null before,
  the same once-only stamp the rating window relies on — and only when its product is public; a reply
  to a review notifies on the first answer and not on an edit.
- **A release goes to everybody holding the product, grouped by account**, in `sendBatch` chunks of
  100, and deliberately regardless of the channel gate: that gate decides who may download, not who
  may know, and every holder is a supporter anyway. Free downloaders are not on the list —
  `download_events` is a record, not a subscription.
- **`amount` is formatted here, with `formatMoney`**, the function the receipt uses, in the
  receipt's language — the consumer would otherwise have to learn that CLP has no minor unit.
- **`notify: false` on a hand-recorded sale or a refund is honoured by the bell too.** It is the
  editor saying "do not tell the buyer", not only "do not email".
- **Publishing never fails the request.** Every call is the tail of something already written, and a
  webhook in particular must answer 200.
