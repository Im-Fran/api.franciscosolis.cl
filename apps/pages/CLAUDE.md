...
- `migrations/` — `0000_init.sql` and `0001_payments_and_downloads.sql`, generated by drizzle-kit.
...
- **The webhook is the only unauthenticated write here, and three things guard it**
  ...
  the payment* with our own credential, because the notification body is not evidence and anybody can
  post one claiming an approval; and a unique `<payment>:<status>` key that makes a retry a no-op which
  still answers 200 — anything else has the provider retrying forever. With no secret configured it
  refuses everything: the endpoint that can grant a licence fails closed.
