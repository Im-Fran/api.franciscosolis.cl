/**
 * The tabs an application page can show, as a registry.
 *
 * This is the "same company standard" the whole Worker exists to enforce: every application built
 * here gets the same bar under its banner, drawn from the same four tabs, in whatever subset and
 * order the editor picks. A new kind of tab is an entry here plus the route that serves it — never
 * a per-application field describing a layout, which is how a set of pages stops being a standard.
 *
 * `overview` is not in the optional set: a page with no Overview is a banner and a row of links,
 * and every other tab is something a visitor reaches *after* deciding the application is for them.
...
const TABS = {
  overview: {
    name: 'Overview',
    description: 'One centred Markdown document: what the application is, and why anyone would want it.',
    source: 'field',
  },
  updates: {
    name: 'Updates',
  ...
  contact: {
    name: 'Contact',
    description: 'How to reach support, as one Markdown document beside the application\'s own links.',
    source: 'field',
  },
...
/** The one tab every application has, whatever else it turns on. */
const REQUIRED_TAB: TabKey = 'overview'

const isTab = (value: string): value is TabKey => value in TABS

/**
 * What actually gets stored for an application's `tabs`.
 *
 ...
 * has one tab fewer" instead of 500-ing the application it was left on.
 */
