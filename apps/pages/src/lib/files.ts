 ...
 *
 * A file is attached to a release rather than to the application: that is what makes "the archive of
 * past versions" a consequence of the changelog instead of a second list to keep in step with it,
 * and it is why the Updates tab is where an editor uploads.
 */
 ...
 * application prefix means everything one application ever published can be listed — and deleted —
 ...
 */
const objectKeyFor = (applicationId: string, updateId: string, fileId: string) =>
  `releases/${applicationId}/${updateId}/${fileId}`
