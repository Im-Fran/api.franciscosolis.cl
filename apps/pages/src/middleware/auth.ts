...
const allowedDomains = (env: Env) =>
  env.PAGES_ALLOWED_EMAIL_DOMAINS.split(',')
 ...
 * by its provider, and that address belongs to an allowed domain. The domain check is the actual
 * access rule — these pages are edited by @franciscosolis.cl staff, and everyone else is refused
 * even with a flawless token.
 *
 * The accepted audience is the CMS's client id: the editor for this Worker is a section of the CMS
 * interface (`franciscosolis.cl/cms/pages`) rather than an application of its own, so a token good
 * for the CMS is the token that arrives here.
 */
const requireEditor = createMiddleware<AppEnv>(async (c, next) => {
  ...
  if (!isAllowedEmail(c.env, claims.email)) {
    throw new HTTPException(403, { message: 'This account is not allowed to edit application pages' })
  }
