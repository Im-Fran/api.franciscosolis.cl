 ...
 *
 * There is exactly one caller — `POST /admin/translate`, which drafts one prose field of an
 * application page, a release note or a wiki page in one other language — and it is never on the
 * path that saves a record. A model outage, a timeout or an answer that could not be read costs
