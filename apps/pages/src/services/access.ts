...
/**
 * Whether this caller may download this application, and what the page has to show them first.
 *
...
type Access = {
  ...
  has_paid: boolean
  /**
   * Whether the payment modal must be shown. True for every non-payer of a paying application,
...
/**
 * Resolves the access of `account` (possibly nobody) to `application`.
 *
 * The cooldown is attached to the *caller* rather than to the application: it is five seconds for a
 * non-payer and nothing for somebody who paid, which is the only difference between the two download
 * experiences once the modal is out of the way. A free application has no payer and no non-payer, so
...
const resolveAccess = async (
  ...
  const purchase = account && pricing.accepts_payment ? await findActivePurchase(db, application.id, account) : null
  ...
  return {
    ...
    // A paid application is downloadable only once it has been paid for; the other two always are.
