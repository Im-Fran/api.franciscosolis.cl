...
import { findUpdateById, type ApplicationUpdate } from '@/services/updates'
...
/** Reads `:updateId` as a release of `application`, so an id from another one is a 404 here. */
const requireUpdate = async (db: Database, application: Application, id: string): Promise<ApplicationUpdate> => {
...
const requireFile = async (db: Database, update: ApplicationUpdate, id: string) => {
  const file = await findReleaseFileById(db, id)
  if (!file || file.updateId !== update.id) {
...
const BASE = '/applications/:applicationId/updates/:updateId/files'
...
app.get(
  ...
  async (c) => {
    ...
    const update = await requireUpdate(db, application, c.req.param('updateId'))

    const rows = await listReleaseFiles(db, { updateId: update.id })
...
app.post(
  ...
  async (c) => {
    ...
    const existing = await listReleaseFiles(db, { updateId: update.id })
    ...
    const row = {
      ...
      objectKey: objectKeyFor(application.id, update.id, id),
    ...
    await recordAudit(db, {
      ...
      ...getActorContext(c),
      ...getRequestContext(c),
      resourceType: 'application_release_files',
      resourceId: id,
      metadata: { application: application.slug, version: update.version, filename: row.filename },
    })
...
app.put(
  ...
  describeRoute({
    ...
    responses: {
      ...
      401: { description: 'Missing or invalid access token' },
      404: { description: 'No such application, release or file' },
  ...
  async (c) => {
    ...
    const application = await requireApplication(db, c.req.param('applicationId'))
    const update = await requireUpdate(db, application, c.req.param('updateId'))
    const file = await requireFile(db, update, c.req.param('id'))
    ...
    await db
      ...
      .where(eq(applicationReleaseFiles.id, file.id))
    ...
    await recordAudit(db, {
      ...
      resourceType: 'application_release_files',
      resourceId: file.id,
      metadata: { application: application.slug, version: update.version, filename: file.filename, size: object.size },
    })
...
app.patch(
  ...
  describeRoute({
    description:
      'Updates a build\'s metadata. Renaming only changes what a browser saves the file as — the object key is built from ids, so no bytes move. A file with no upload yet cannot be published: the Updates tab would show a download that 404s.',
    tags: ['Admin · Downloads'],
    security: [{ bearerAuth: [] }],
    responses: {
  ...
  async (c) => {
    ...
    const application = await requireApplication(db, c.req.param('applicationId'))
    const update = await requireUpdate(db, application, c.req.param('updateId'))
    const current = await requireFile(db, update, c.req.param('id'))
    ...
    try {
      await db
        ...
        .where(eq(applicationReleaseFiles.id, current.id))
    } catch (error) {
    ...
    await recordAudit(db, {
      ...
      metadata: { application: application.slug, version: update.version, fields: Object.keys(body), status },
    })
...
app.delete(
  ...
  async (c) => {
    ...
    await c.env.RELEASES.delete(file.objectKey)
    await db.delete(applicationReleaseFiles).where(eq(applicationReleaseFiles.id, file.id))
    ...
    await recordAudit(db, {
      ...
      resourceType: 'application_release_files',
      resourceId: file.id,
      metadata: { application: application.slug, version: update.version, filename: file.filename },
    })
