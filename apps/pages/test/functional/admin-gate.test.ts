 ...
 * section of the CMS interface rather than an application of its own.
 */
const admin = (path = '/admin/me', init: RequestInit = {}) =>
  SELF.fetch(`https://pages.test${path}`, init)
...
describe('the editorial gate', () => {
  it('lets a verified @franciscosolis.cl editor in', async () => {
    ...
    const { data } = await response.json<{ data: { email: string; application_id: string } }>()
    expect(data.email).toBe('fran@franciscosolis.cl')
    expect(data.application_id).toBe('franciscosolis-cms')
  })
  ...
  it('refuses a token signed by a key the auth service never published', async () => {
    ...
    stubJwks([publicJwk])
    const stranger = await generateKeyPair('pages-test-key')
...
describe('every editorial route is behind the gate', () => {
  it.each([
    ...
    ['GET', '/admin/audit'],
    ['GET', '/admin/applications/whatever/updates'],
    ['GET', '/admin/applications/whatever/wiki'],
