...
const checkout = async (slug: string, body: Record<string, unknown> = {}, headers?: Record<string, string>) => {
  const response = await call(`/applications/${slug}/checkout`, {
    method: 'POST',
...
describe('POST /applications/:slug/checkout', () => {
  beforeEach(clearDatabase)

  it('opens a pending purchase before the browser leaves, and hands back the checkout URL', async () => {
    await seedApplication({ slug: 'openbattery', name: 'OpenBattery', pricingMode: 'paid', priceAmount: 4990 })
  ...
  it('sends the buyer back to the website, never to the API', async () => {
    await seedApplication({ slug: 'openbattery', pricingMode: 'paid', priceAmount: 4990 })
    const calls = stubMercadoPago()

    await checkout('openbattery')

    expect(calls[0]?.body.back_urls).toMatchObject({ success: 'https://site.test/application/openbattery' })
    expect(calls[0]?.body.notification_url).toBe('https://api.test/pages/payments/mercadopago/webhook')
  })

  it('charges a paid application its price even when a smaller amount is asked for', async () => {
    await seedApplication({ slug: 'openbattery', pricingMode: 'paid', priceAmount: 4990 })
    stubMercadoPago()
  ...
  it('lets a donor name their own amount', async () => {
    await seedApplication({ slug: 'freebie', pricingMode: 'donation', suggestedAmount: 2000 })
    stubMercadoPago()
  ...
  it('refuses to charge for a free application', async () => {
  ...
  it('refuses to sell a paid application twice', async () => {
  ...
  it('lets somebody donate again, because a donation is a repeatable act', async () => {
    const application = await seedApplication({ slug: 'freebie', pricingMode: 'donation', suggestedAmount: 1000 })
    await seedPurchase({ applicationId: application.id, applicationSlug: application.slug, kind: 'donation' })
    stubMercadoPago()
  ...
  it('needs an account: a payment with nobody on it has nowhere to live', async () => {
    ...
    const response = await call('/applications/openbattery/checkout', { method: 'POST', body: '{}' })
...
describe('GET /me/purchases', () => {
  ...
  it('lists what the account paid for, newest first', async () => {
    const first = await seedApplication({ slug: 'one', pricingMode: 'paid', priceAmount: 1000 })
    const second = await seedApplication({ slug: 'two', pricingMode: 'paid', priceAmount: 2000 })
    await seedPurchase({ applicationId: first.id, applicationSlug: first.slug, createdAt: new Date(1_700_000_000_000) })
    await seedPurchase({ applicationId: second.id, applicationSlug: second.slug, createdAt: new Date(1_800_000_000_000) })

    const response = await call('/me/purchases', { headers: await asBuyer() })
    const { data } = (await response.json()) as { data: { application_slug: string; active: boolean }[] }

    expect(data.map((row) => row.application_slug)).toEqual(['two', 'one'])
    expect(data.every((row) => row.active)).toBe(true)
  })

  it('never shows somebody else\'s payments', async () => {
    const application = await seedApplication({ slug: 'one', pricingMode: 'paid', priceAmount: 1000 })
    await seedPurchase({
