...
import { clearDatabase, seedApplication, seedPurchase, seedReleaseFile, seedUpdate } from '../helpers/db'
import { asBuyer } from '../helpers/tokens'

const call = (path: string, init: RequestInit = {}) => SELF.fetch(`https://pages.test${path}`, init)

/** A paid application with one published build behind it — the shape every test here starts from. */
const paidRelease = async (overrides: Record<string, unknown> = {}) => {
  ...
  const file = await seedReleaseFile({ applicationId: application.id, updateId: update.id, filename: 'app-2.6.4.jar' })
  return { application, update, file }
}

const ticketFor = async (slug: string, fileId: string, headers: Record<string, string> = {}) => {
  const response = await call(`/applications/${slug}/files/${fileId}/download`, { method: 'POST', headers })
 ...
 *
 * A minted URL is the public one — `PAGES_PUBLIC_URL` plus the path — and the public one carries the
...
describe('GET /applications/:slug/updates/:version/files', () => {
  beforeEach(clearDatabase)

  it('lists the builds of a published release and says whether paying comes first', async () => {
    await paidRelease()

    const response = await call('/applications/openbattery/updates/2.6.4/files')
  ...
  it('never exposes the object key or a bucket URL', async () => {
    ...
    const text = await (await call('/applications/openbattery/updates/2.6.4/files')).text()
  ...
  it('leaves out a file whose bytes were never uploaded', async () => {
    const application = await seedApplication({ slug: 'openbattery' })
    const update = await seedUpdate({ applicationId: application.id, version: '1.0' })
    await seedReleaseFile({ applicationId: application.id, updateId: update.id, uploadedAt: null })

    const { data } = (await (await call('/applications/openbattery/updates/1.0/files')).json()) as {
      data: { files: unknown[] }
    }
  ...
  it('does not exist for a draft release', async () => {
    const application = await seedApplication({ slug: 'openbattery' })
    const update = await seedUpdate({ applicationId: application.id, version: '3.0', status: 'draft' })
    await seedReleaseFile({ applicationId: application.id, updateId: update.id })

    expect((await call('/applications/openbattery/updates/3.0/files')).status).toBe(404)
  })
})

describe('POST /applications/:slug/files/:id/download', () => {
  beforeEach(clearDatabase)

  it('refuses a paid application with 402 until it has been paid for', async () => {
  ...
  it('mints a ticket that works immediately for somebody who paid', async () => {
    const { application, file } = await paidRelease()
    await seedPurchase({ applicationId: application.id, applicationSlug: application.slug })
  ...
  it('mints a ticket behind a five-second cooldown for a non-payer of an optional-pay application', async () => {
    const application = await seedApplication({ slug: 'freebie', pricingMode: 'donation', suggestedAmount: 2000 })
    const update = await seedUpdate({ applicationId: application.id, version: '1.0' })
  ...
  it('refuses a file reached through another application\'s path', async () => {
    const { file } = await paidRelease()
    await seedApplication({ slug: 'elsewhere' })
  ...
  it('refuses a build hanging off a release that is still a draft', async () => {
    const application = await seedApplication({ slug: 'gratis' })
    const update = await seedUpdate({ applicationId: application.id, version: '9.9', status: 'draft' })
    const file = await seedReleaseFile({ applicationId: application.id, updateId: update.id })
...
describe('GET /downloads/:ticket', () => {
  ...
  it('serves the bytes, names the file and never lets a shared cache keep them', async () => {
    ...
    const file = await seedReleaseFile(
      { applicationId: application.id, updateId: update.id, filename: 'gratis-1.0.zip' },
      'the-actual-bytes',
    )
  ...
  it('records the download against the account that made it', async () => {
    ...
    const row = await env.DB.prepare(
      'SELECT user_id, paid, version, filename, application_slug FROM download_events',
    ).first<{ user_id: string; paid: number; version: string; filename: string; application_slug: string }>()
  ...
  it('counts the download on the file', async () => {
    ...
    const row = await env.DB.prepare('SELECT download_count FROM application_release_files WHERE id = ?')
      .bind(file.id)
  ...
  it('stops working once the file is unpublished, ticket in hand or not', async () => {
    ...
    const { body } = await ticketFor('gratis', file.id)
    await env.DB.prepare('UPDATE application_release_files SET status = ? WHERE id = ?').bind('draft', file.id).run()
  ...
  it('honours a range request so a download can be resumed', async () => {
    const application = await seedApplication({ slug: 'gratis' })
    const update = await seedUpdate({ applicationId: application.id, version: '1.0' })
    const file = await seedReleaseFile({ applicationId: application.id, updateId: update.id }, '0123456789')
