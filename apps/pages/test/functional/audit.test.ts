...
const admin = async (path: string, init: RequestInit = {}) =>
  SELF.fetch(`https://pages.test/admin${path}`, {
    ...init,
...
describe('GET /admin/audit', () => {
  ...
  it('reports every write, newest first', async () => {
    ...
    await admin(`/applications/${app.id}`, { method: 'PATCH', body: JSON.stringify({ name: 'OpenBattery' }) })
    await admin(`/applications/${app.id}/wiki`, { method: 'POST', body: JSON.stringify({ title: 'Installation' }) })

    const data = await json<{ event: string }[]>(await admin('/audit'))

    expect(data.map((row) => row.event)).toEqual(['wiki.created', 'application.updated'])
  })
  ...
  it('names the fields a patch touched, without echoing their values', async () => {
    const app = await seedApplication({ slug: 'openbattery' })
    await admin(`/applications/${app.id}`, {
      method: 'PATCH',
