...
describe('GET /', () => {
  it('reports the service is up', async () => {
    const response = await SELF.fetch('https://pages.test/')
    const { code, data } = await response.json<RootPayload>()

    expect(response.status).toBe(200)
    expect(code).toBe(200)
    expect(data.message).toBe('Hello, Standalone App Pages!')
  })
  ...
  it('advertises the tabs a page can be built from, with their descriptions', async () => {
    const { data } = await (await SELF.fetch('https://pages.test/')).json<RootPayload>()

    expect(data.tabs.map((tab) => tab.key)).toEqual(['overview', 'updates', 'wiki', 'contact'])
...
describe('GET /openapi.json', () => {
  it('describes the service, and does not document itself', async () => {
    const response = await SELF.fetch('https://pages.test/openapi.json')
    const document = await response.json<{ info: { title: string }; paths: Record<string, unknown> }>()

    expect(response.status).toBe(200)
    expect(document.info.title).toBe('FranciscoSolis - Standalone App Pages API')
    expect(Object.keys(document.paths)).toContain('/applications/{slug}')
