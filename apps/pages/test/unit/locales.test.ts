...
describe('localize', () => {
  ...
  it('overlays only the fields that were translated, leaving the rest to fall back', () => {
    const result = localize(base, { es: { name: 'Batería Abierta' } }, 'es', APPLICATION_TRANSLATABLE_FIELDS)
  ...
  it('ignores an override that is empty, which is an editor clearing a translation', () => {
    const result = localize(base, { es: { name: '   ' } }, 'es', APPLICATION_TRANSLATABLE_FIELDS)

    expect(result.name).toBe('OpenBattery')
  })

  it('leaves the row alone when the default locale is asked for', () => {
    const result = localize(base, { es: { name: 'Batería Abierta' } }, 'en', APPLICATION_TRANSLATABLE_FIELDS)

    expect(result.name).toBe('OpenBattery')
  })

  /**
   * The field set is per kind of row, and that is the whole reason it is a parameter: an update
  ...
  it('only applies the fields the caller said belong to this kind of row', () => {
    const update = { title: 'Full 1.21.11 support', body: 'Changes' }
