...
const REQUIRED_VARS = [
  'AUTH_JWKS_URL',
  'AUTH_ISSUER',
  'PAGES_ALLOWED_AUDIENCES',
  'PAGES_ALLOWED_EMAIL_DOMAINS',
  'PAGES_ACCOUNT_AUDIENCES',
  'PAGES_PUBLIC_URL',
  'SITE_BASE_URL',
...
describe('the test environment', () => {
  ...
  it('keeps the two audience lists distinct', () => {
    ...
    expect(env.PAGES_ALLOWED_AUDIENCES).not.toBe(env.PAGES_ACCOUNT_AUDIENCES)
    expect(env.PAGES_ALLOWED_AUDIENCES.split(',')).not.toContain('franciscosolis-web')
  })
  ...
  it('never points the token gate or the provider callbacks at production', () => {
    ...
    expect(env.PAGES_PUBLIC_URL).not.toContain('franciscosolis.cl')
