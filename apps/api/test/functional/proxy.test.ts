...
const MODULES: ModuleName[] = ['landing', 'auth', 'cms', 'pages', 'support']

/** The binding name each proxied prefix forwards to. */
const BINDING_OF: Record<ModuleName, keyof Env> = { landing: 'LANDING', auth: 'AUTH', cms: 'CMS', pages: 'PAGES', support: 'SUPPORT' }
 ...
 * `/auth/*`, `/cms/*` and `/pages/*` forward the caller's Request as it stands. That difference is deliberate —
...
describe('header forwarding differs per module', () => {
  ...
  it.each(['auth', 'cms', 'pages', 'support'])('forwards the whole request to %s', async (module) => {
  ...
  it.each(['auth', 'cms', 'pages', 'support'])('does not drop the audit headers %s needs', async (module) => {
