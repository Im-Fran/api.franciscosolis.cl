...
const MODULES: ModuleName[] = ['landing', 'auth', 'cms', 'pages']

/** The binding name each proxied prefix forwards to. */
const BINDING_OF: Record<ModuleName, keyof Env> = { landing: 'LANDING', auth: 'AUTH', cms: 'CMS', pages: 'PAGES' }
...
describe('header forwarding differs per module', () => {
  ...
  it.each(['auth', 'cms', 'pages'])('forwards the whole request to %s', async (module) => {
  ...
  it.each(['auth', 'cms', 'pages'])('does not drop the audit headers %s needs', async (module) => {
