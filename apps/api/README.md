...
It's built with [Hono](https://hono.dev/) on [Cloudflare Workers](https://workers.cloudflare.com/), using `wrangler` for both local development and deployment. Six internal components are registered — `landing` (the landing site's Worker, at `apps/landing`), `auth` (centralized authentication, at `apps/auth`), `cms` (content management, at `apps/cms`), `marketplace` (the storefront, at `apps/marketplace`, plus its deprecated `/pages/*` alias), `support` (tickets and the help centre, at `apps/support`) and `notifications` (the notification centre, at `apps/notifications`) — and they're declared as data in a single registry (`src/services.ts`), so adding a new internal module is one entry in that array rather than new code in the gateway.
...
- **Proxies to `marketplace`, `support` and `notifications` (`ALL /marketplace/*`, `/support/*`, `/notifications/*`)** — Each forwards the complete request, like the `cms` proxy. The notifications module is reached only from here; the Workers that *produce* notifications publish onto a Cloudflare Queue instead, so this gateway is not on that path.
...
- **Brand lockup (`GET /brand/lockup.png`)** — Serves the FranciscoSolis horizontal lockup as a 400×66 PNG, cached for a week behind a content-derived ETag. It is the only content this gateway owns, and it exists because every email `apps/auth` and `apps/cms` send points an `<img>` at this URL: Gmail blocks `data:` URIs and strips inline SVG, so an email logo has to be a real HTTP asset, and this Worker holds the only public hostname in the monorepo.
- **Locked-down CORS** — Only allows `GET`, `POST`, `PUT`, `PATCH`, `DELETE` and `OPTIONS` from `localhost:5173`, `*.franciscosolis.workers.dev`, and `*.franciscosolis.cl`; any other origin gets the response meant for `https://franciscosolis.cl`. The suffixes are matched on a dot boundary, over HTTPS on the default port, so a domain that merely *ends* with one of ours (`evilfranciscosolis.cl`) is not ours. `*.franciscosolis.workers.dev` is the account's own `workers.dev` subdomain, which is where Cloudflare serves branch and version previews. The write verbs exist for the auth module's sign-in, token exchange and admin API. See `src/cors.ts`.
...
The service bindings (`LANDING`, `AUTH`, `CMS`, `MARKETPLACE`, `SUPPORT` and `NOTIFICATIONS`, each naming the Worker of the same lowercase name — and the `-dev` one under `env.dev`) connect this Worker to the internal Workers deployed in the same Cloudflare account — the proxies and the OpenAPI merge depend on those Workers being deployed and reachable under exactly those names, which is why this Worker is deployed last.
