...
It's built with [Hono](https://hono.dev/) on [Cloudflare Workers](https://workers.cloudflare.com/), using `wrangler` for both local development and deployment. Currently the only registered component is `landing` (the landing site's Worker, at `apps/landing` in the monorepo), but the code is designed so that adding a new internal module is a matter of a few lines.
...
- **Aggregated OpenAPI (`GET /openapi.json`)** — Generates this API's own OpenAPI spec with `hono-openapi` and merges it (`mergeRemoteSpecs`) with the spec exposed by each internal component (currently `landing`, mounted under the `/landing` prefix). If an internal component doesn't respond, it's silently skipped from the merged spec instead of breaking the rest.
- **Locked-down CORS** — Only allows `GET` from `localhost:5173`, `*.franciscosolis.workers.dev`, and `*.franciscosolis.cl`; any other origin gets the response meant for `https://franciscosolis.cl`.
...
- A Cloudflare account with access to the project (Workers + Service Bindings) for `deploy`, and for the `LANDING` binding to resolve in production
...
> From the monorepo root, `pnpm dev` runs the `dev` script of **all** workspace apps (`apps/*`) in parallel, including `landing`, which is needed so the `/landing/*` proxy has something to forward to locally.
...
The `LANDING` binding (`services: [{ binding: "LANDING", service: "landing" }]`) connects this Worker to the `landing` Worker deployed in the same Cloudflare account — the `/landing/*` proxy and the OpenAPI merge depend on that Worker being deployed and reachable.
