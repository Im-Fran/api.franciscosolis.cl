 ...
 *
 * The route's contract is unusual for this Worker and the tests are shaped around it: a model that
...
const call = (body: unknown, init?: RequestInit) =>
  SELF.fetch('https://cms.internal/admin/translate', {
    method: 'POST',
    headers,
...
describe('POST /admin/translate', () => {
  ...
  it('writes nothing to the record it translated for', async () => {
    ...
    const entries = await env.DB.prepare('SELECT COUNT(*) AS total FROM content_entries').first<{ total: number }>()
  ...
  it('refuses a draft the API would reject on save', async () => {
    // Over the 200-character cap on a title. Offering it would hand the editor a draft they cannot
    // keep, which is worse than offering nothing.
    stubAiResponse(aiTranslation('á'.repeat(201)))
  ...
  it('is behind the editor gate', async () => {
    const response = await SELF.fetch('https://cms.internal/admin/translate', {
      method: 'POST',
