 ...
 * a request would answer `401 Invalid access token: JWKS endpoint answered 522` to every
 ...
 * simply shut.
 *
 * The trade-off the offline check buys: a token is a snapshot from when it was minted, so a revoked
 * account keeps reading its inbox for at most one access-token lifetime (15 minutes). That is
 * harmless here — every row this Worker serves already belongs to the token's own `sub`.
 */
 ...
 *
 * The `aud` allowlist matters as much as the signature — a token minted for the CMS is a valid
 * token, it is just not one this Worker reads an inbox for.
 */
const verifyAccessToken = async (
  env: Env,
  token: string,
  allowedAudiences: string = env.NOTIFICATIONS_ALLOWED_AUDIENCES,
