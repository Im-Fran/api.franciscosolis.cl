...
together three Cloudflare Workers, all living directly in this repo:
...
implementation of a Worker, read/edit inside `apps/api`, `apps/landing` or `apps/auth` —
the root repo only owns workspace-wide wiring (pnpm workspace/catalog, root scripts).
...
- Every Worker uses Hono + hono-openapi + valibot + axios + Wrangler, all pinned via a
  ...
  those deps, add/bump them in the catalog instead. `apps/auth` additionally uses
  drizzle-orm/drizzle-kit, also catalogued.
...
- **Auth is the only stateful Worker**: `apps/auth` owns the `franciscosolis_auth` D1
  database and is the only app with an ORM (Drizzle) and migrations. It issues EdDSA-signed
  JWTs that any other Worker can verify offline against
  `https://api.franciscosolis.cl/auth/.well-known/jwks.json` — never add a service binding
  back into `auth` just to validate a token.
...
- `apps/landing/.dev.vars` holds the `GH_TOKEN` secret for local dev, and
  ...
  ever be committed (both already gitignored).
