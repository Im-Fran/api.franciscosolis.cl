...
- Every Worker uses Hono + hono-openapi + valibot + Wrangler, all pinned via a
  ...
  `apps/support` is the only one with `postal-mime`, and the only one using Workers AI and Vectorize.
  `apps/pages` talks to MercadoPago over plain `fetch` rather than through an SDK — a handful of
  functions in `src/lib/mercadopago.ts` against a dependency that assumes Node.
